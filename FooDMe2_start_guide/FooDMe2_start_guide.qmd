...
::: {.callout-note}
If you are running the pipeline behind a proxy server, some additional configuration is required when using one of the container engines (singularity, apptainer, docker).
:::

First, make sure that the proxy information is properly set using:

```bash
echo $HTTPS_PROXY
```

This should print the proxy configuration to the screen. If not, please check with your IT department. 

Next, make sure this variable can be seen by the container by adding the `envWhitelist` option to your config (only showing part of the config here):


::: {.panel-tabset group="proxies"}
...
apptainer {
	enabled = true
	cacheDir = "$HOME/nextflow/envs"
	envWhiteList = "HTTPS_PROXY,HTTP_PROXY"
}
```
